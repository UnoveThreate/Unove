  ...
  <body>
      ...
      <c:forEach var="row" begin="1" end="9">
        <c:forEach var="col" begin="1" end="18">
          ...
          <c:forEach var="seat" items="${seats}">
            <c:if test="${seat.coordinateX == row && seat.coordinateY == col}">
          ...
          <c:choose>
            <c:when test="${currentSeat != null}">
              <div
                class="seat ${currentSeat.available ? 'available' : 'unavailable'}"
    ...
    >
      <input
        type="hidden"
        name="movieSlotID"
        value="${selectedSlot.movieSlotID}"
      />
      <input type="hidden" id="selectedSeatID" name="selectedSeatID" value="" />
      <button type="submit" disabled id="bookButton" class="book-button">
        Đặt Ghế
      </button>
    </form>
