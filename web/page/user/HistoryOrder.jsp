    ...
    <div class="container">
          ...
          <tbody>
            <c:forEach var="order" items="${historyOrders}">
              <tr>
                ...
                <td>
                  <c:if test="${not empty order.CanteenItems}">
                    <ul class="canteen-items">
                      <c:forEach
                        var="canteenItem"
                        items="${order.CanteenItems}"
                      >
                        <li>
                          ${canteenItem.CanteenItemName} - Số lượng:
                          ${canteenItem.Quantity}
                        </li>
                      </c:forEach>
                    </ul>
                  </c:if>
                ...
                <td>
                  <a
                    href="OrderHistoryDetailServlet?orderID=${order.OrderID}"
                    class="detail-link"
                    >Xem chi tiết</a
                  >
                </td>
      ...
      <div style="text-align: center; margin-top: 30px">
        <form action="${pageContext.request.contextPath}/history" method="post">
          <button type="submit" name="action" value="back" class="back-button">
            Trở về trang chủ
          </button>
        </form>
      </div>
